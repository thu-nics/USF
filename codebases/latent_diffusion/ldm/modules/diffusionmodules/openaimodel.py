...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        ...
        if os.path.exists(f"temp/skip_coefficients_{pid}.pth"):
            skip_coefficients = th.load(f"temp/skip_coefficients_{pid}.pth",weights_only=True)
        else:
            print("Warning! don't get skip coefficients, use 1 instead")
            skip_coefficients = th.linspace(1.0, 1.0, 12)
        assert len(skip_coefficients) == len(hs)
        skip_coefficients = skip_coefficients.to(x.device)
        skip_index = -1
        ###
        for module in self.output_blocks:
            h = th.cat([h, hs.pop()*skip_coefficients[skip_index]], dim=1)
            skip_index -= 1
            h = module(h, emb, context)
        h = h.type(x.dtype)
        if self.predict_codebook_ids:
