...
class DPM_Solver_v3:
    def __init__(
        ...
        self.steps = steps
        t_0 = 1.0 / self.noise_schedule.total_N if dpmsolver_v3_t_end is None else dpmsolver_v3_t_end
        t_T = self.noise_schedule.T if dpmsolver_v3_t_start is None else dpmsolver_v3_t_start
        assert (
            t_0 > 0 and t_T > 0
        ...
        timesteps = self.get_time_steps("logSNR", t_T, t_0, self.statistics_steps, "cpu")
        ...
        if skip_type == "logSNR" or skip_type == "time_uniform" or skip_type == "time_quadratic":
            self.timesteps = self.get_time_steps(skip_type, t_T=t_start, t_0=t_end, N=steps, device=device)
    ...
    def multistep_predictor_update(self, x_lst, eps_lst, time_lst, index_lst, t, i_t, order=1, pseudo=False):
        ...
        # print(f"Predictor order: {order}")
        # print(f"active points: {indexes}")
        # print(f"start point: {indexes[0]}")
    ...
    def sample(
        ...
        for step in range(1, steps + 1):
            cached_x.append(x)
            if step == 1:
                cached_model_output.append(x/80)
            else:
