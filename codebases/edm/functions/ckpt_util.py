...
from models.score_based_model.ncsnpp import NCSNpp
from models.score_based_model.ema import ExponentialMovingAverage
from models.edm.networks import EDMPrecond
...
def load_ckpt(model, config, map_location):
    if "ckpt_dir" in config.model.__dict__.keys():
        ...
        elif config.model.model_type in SCORE_MODELS:
        ...
        if config.model.model_type not in SCORE_MODELS:
...
def load_model_for_sample(config, device, gpus):
    map_location = device
    model = create_model(config)
    model = model.to(device)
    if config.model.model_type in SCORE_MODELS or getattr(config.model, "parallel", False):
        model = torch.nn.DataParallel(model, device_ids=gpus, output_device=gpus[0])
    load_ckpt(model, config, map_location)
    model.eval()
    if config.sampling.cond_class and not config.model.is_upsampling:
        classifier = load_classifier(config, map_location)
