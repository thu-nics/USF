...
class Diffusion(object):
    ...
    def compute_nfe(self, decisions=None):
        ...
        if decisions.get("orders", None) is not None:
            ...
            if afs != "no_afs":
                ...
                nfe = steps
            for corrector_type in decisions["corrector_types"]:
    ...
    # temp experiment functions
    def generate_baseline_population(self):
        ...
        from uni_sampler.uni_sampler import uni_sampler
        from uni_sampler.utils import get_empirical_decisions, print_decisions
        import random
        noise_schedule = NoiseScheduleEDM()
        sampler = uni_sampler(None, noise_schedule, statistics_dir=self.config.sampling.statistics_dir, dpmsolver_v3_t_start=80, dpmsolver_v3_t_end=0.002)
        id = 0
        skip_type = "logSNR"
        for baseline in baseline_types:
            # for afs in ["no_afs","zero_x0","average_x0"]:
            for afs in ["no_afs"]:
                if baseline in ["dpmsolver","dpmsolver++"]:
                    # order = args.dpm_solver_order
                    # method = args.dpm_solver_method # single step or multi step
                    # solver_type = args.dpm_solver_type # taylor or dpm-solver
                    for t in timesteps:
                        dict_args = {
                            "uni_sampler_decision_type":baseline,
                        ...
                        dict_args = dict2namespace(dict_args)
                        decision = get_empirical_decisions(dict_args, sampler, self.device)
                        self.logger.info("using empirical decisions:")
                        print_decisions(self.logger, decision)
                        for derivate_type in decision["derivative_types"]:
                            for order in derivate_type:
                                if order["relaxation"] is not None:
                                    order["relaxation_coefficient"] = relaxation_dict[order["relaxation"]]
                                    if baseline == "dpmsolver++":
                                        order["relaxation_coefficient"] *= -1
                                    order["relaxation"] = "linear"
                        fid = self.sample(number_of_samples=1000, decisions=decision)
                        torch.save([decision, fid], os.path.join("./search_data/initialize/", f"{baseline}_{t}_{afs}_{fid}.pth"))
                elif baseline=="unipc":
                    # pass
                    ...
                    for t in timesteps:
                        for uni_pc_prediction_type in ["data_prediction","noise_prediction"]:
                            dict_args = {
                                "uni_sampler_decision_type":baseline,
                                "uni_pc_prediction_type":uni_pc_prediction_type,
                                "uni_pc_disable_corrector":True,
                                "skip_type":skip_type,
                                "uni_pc_order":3,
                                "timesteps":t,
                                "afs":afs,
                                ...
                                "timesteps":t,
                                "lower_order_final":True,
                                "uni_pc_variant":random.choice(["bh1","bh2","vary_coeff"])
                            }
                            dict_args = dict2namespace(dict_args)
                            decision = get_empirical_decisions(dict_args, sampler, self.device)
                            self.logger.info("using empirical decisions:")
                            ...
                            for derivate_type in decision["derivative_types"]:
                                for order in derivate_type:
                                    if order["relaxation"] is not None:
                                        order["relaxation_coefficient"] = relaxation_dict[order["relaxation"]]
                                        if uni_pc_prediction_type == "data_prediction":
                                            order["relaxation_coefficient"] *= -1
                                        order["relaxation"] = "linear"
                            fid = self.sample(number_of_samples=1000, decisions=decision)
                            torch.save([decision, fid], os.path.join("./search_data/initialize/", f"{baseline}_{t}_{afs}_{fid}.pth"))
                elif baseline=="dpmsolver_v3":
                        ...
                        for derivate_type in decision["derivative_types"]:
                            for order in derivate_type:
                                if order["relaxation"] is not None:
                                    order["relaxation_coefficient"] = relaxation_dict[order["relaxation"]]
                                    order["relaxation"] = "linear"
                        fid = self.sample(number_of_samples=1024, decisions=decision)
                        torch.save([decision, fid], os.path.join("./search_data/initialize/", f"{baseline}_{t}_{afs}_{fid}.pth"))
                else:
                    raise NotImplementedError