...
class uni_sampler:
    def __init__(
        ...
        self.statistics_init(statistics_dir, t_start=dpmsolver_v3_t_start, t_end=dpmsolver_v3_t_end)
    ...
    def update(self, x_prev_list, model_prev_list, t_prev_list, t, taylor_order, prediction_type, start_point, derivative_types, corrector=False):
        ...
        for t_prev in t_prev_list:
            lambda_prev_list.append(ns.marginal_lambda(t_prev) if t_prev is not None else None)
        ...
        if prediction_type == "noise_prediction":
            ...
            for t_prev in t_prev_list:
                if t_prev is None:
                    ...
                    lambda_prev_list.append(ns.marginal_lambda(t_prev))
                    alpha_prev_list.append(ns.marginal_alpha(t_prev))
    ...
    def statistics_init(self, statistics_dir, degenerated=False, t_start=1, t_end=1e-4):
        ...
        if degenerated:
            ...
            s = np.zeros_like(s)
            b = np.zeros_like(b)
        # l_data = np.ones_like(l)
        # s_data = np.zeros_like(s)
        # b_data = np.zeros_like(b)
        # l = np.zeros_like(l)
        # s = -np.ones_like(s)
        # b = np.zeros_like(b)
        # noise, data = 1, 0
        # l = data*l_data + noise*l_noise + (1-data-noise)*l
        # s = data*s_data + noise*s_noise + (1-data-noise)*s
        # b = data*b_data + noise*b_noise + (1-data-noise)*b
