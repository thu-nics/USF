        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            if solver_type == 'dpmsolver':
                x_t = (
                    torch.exp(log_alpha_t - log_alpha_s) * x
                    - (sigma_t * phi_1) * model_s
                    - (0.5 / r1) * (sigma_t * phi_1) * (model_s1 - model_s) # TODO: delete it after test. (0.5 * h / (lambda_s1 - lambda_s)) * (sigma_t * phi_1) * (model_s1 - model_s)
                )
            elif solver_type == 'taylor':
                x_t = (
                    torch.exp(log_alpha_t - log_alpha_s) * x
        ...
        with torch.no_grad():
            if method == 'adaptive':
                ...
                for step, order in enumerate(orders):
                    ...
                    x = self.singlestep_dpm_solver_update(x, s, t, order, return_intermediate=return_intermediate, solver_type=solver_type, r1=r1, r2=r2)
                    if self.correcting_xt_fn is not None:
                        x = self.correcting_xt_fn(x, t, step)
                    if return_intermediate:
                        self.intermediates.append(x)
